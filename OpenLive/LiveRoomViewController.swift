...
// ---------- 哎吖科技添加 开始 ---------
        NotificationCenter.default.addObserver(self, selector: #selector(licenseMessage(_:)), name: NSNotification.Name.AiyaLicense, object: nil)
        
        AYLicenseManager.initLicense("cc13acdbebf941af99a749aa505a2e05")
        
        videoProcess.registerVideoFrameObserver(rtcEngine)
        videoProcess.setEffectPath(Bundle.main.path(forResource: "meta", ofType: "json", inDirectory: "mogulin"))
// ---------- 哎吖科技添加 结束 ---------
    }
...
private extension LiveRoomViewController {
    @objc func licenseMessage(_ notification: Notification) {
        
        let result = AiyaLicenseResult.init(rawValue: notification.userInfo![AnyHashable(AiyaLicenseNotificationUserInfoKey)] as! UInt)!
        
        switch (result) {
        case .success:
            NSLog("哎吖科技 License 验证成功");
            break;
            
        case .fail:
            NSLog("哎吖科技 License 验证失败");
            break;
        }
    }
}
...
private extension LiveRoomViewController {
    func loadAgoraKit() {
        ...
        if isBroadcaster {
            rtcEngine.setCameraCapturerConfiguration(AgoraCameraCapturerConfiguration())
